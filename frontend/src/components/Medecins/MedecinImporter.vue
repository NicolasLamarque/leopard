  ...
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
    <div class="mb-6">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400">
        Importez une liste de médecins depuis un fichier Excel (.xlsx)
      </p>
    </div>
  ...
  <div class="border-2 border-dashed rounded-lg p-8 text-center border-gray-300 dark:border-gray-600 hover:border-blue-400 transition-all">
    ...
    <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
      <label class="flex items-center gap-3">
        ...
        />
        <div>
          <p class="font-medium text-gray-900 dark:text-white">Mettre à jour les médecins existants</p>
          <p class="text-xs text-gray-500 dark:text-gray-400">Si un médecin existe déjà (même numéro de licence), ses informations seront mises à jour</p>
        </div>
      </label>
    </div>
...
const importMedecins = async () => {
  ...
  try {
    ...
    if (updateExisting.value) {
      ...
      result = await window.go.main.App.ImportMedecinsWithUpdate(selectedFilePath.value)
      ...
      result = await window.go.main.App.ImportMedecins(selectedFilePath.value)
