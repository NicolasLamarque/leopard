...
func (db *Database) CreateClient(req models.CreateClientRequest, createdBy int, cryptoSvc *crypto.CryptoService) (int64, error) {
	...
	query := `
		INSERT INTO clients (
			...
			no_dossier_leopard, medecin_famille_No_Licence, pharmacie_id,
	...
	data := map[string]interface{}{
		...
		"pharmacie_id":               encryptedReq.PharmacieID,
...
func (db *Database) UpdateClient(req models.UpdateClientRequest, cryptoSvc *crypto.CryptoService) error {
	...
	query := `
		UPDATE clients SET
			...
			medecin_famille_No_Licence = :medecin_famille_No_Licence, pharmacie_id = :pharmacie_id,
